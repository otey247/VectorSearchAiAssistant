...
if (-Not ($deployments -Contains $completionsDeployment)) {
    ...
    az cognitiveservices account deployment create -g $resourceGroup -n $name --deployment-name $completionsDeployment --model-name 'gpt-35-turbo' --model-version '0613' --model-format OpenAI --sku Standard --sku-capacity 20
}
...
if (-Not ($deployments -Contains $embeddingsDeployment)) {
    ...
    az cognitiveservices account deployment create -g $resourceGroup -n $name --deployment-name $embeddingsDeployment --model-name 'text-embedding-ada-002' --model-version '2' --model-format OpenAI --sku Standard --sku-capacity 20
}
